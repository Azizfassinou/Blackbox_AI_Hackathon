...
"""
        issues = self.scanner.analyze(code, "test.py")
        assert len(issues) > 0
        assert any('password' in issue['message'].lower() or 'api key' in issue['message'].lower() for issue in issues)
...
"""
        issues = self.scanner.analyze(code, "test.py")
        assert len(issues) > 0
        assert any('command injection' in issue['message'].lower() or 'os.system' in issue['message'].lower() for issue in issues)
...
"""
        issues = self.scanner.analyze(code, "test.py")
        assert len(issues) > 0
        assert any('ssl' in issue['message'].lower() or 'verify' in issue['message'].lower() for issue in issues)
