...
"""
    ...
    def _build_critical_issues_section(self, file_analyses: List[Dict[str, Any]]) -> str:
    ...
    def _build_recommendations_section(
        ...
        if security_count > 0:
            recommendations.append("🔒 Review and fix security issues to prevent vulnerabilities")
        ...
        has_docs = any(fa['filename'].endswith(('.md', '.rst', '.txt')) for fa in file_analyses)
        if not has_docs and len(file_analyses) > 3:
            recommendations.append("📚 Update documentation to reflect changes")
        
        if not recommendations:
            recommendations.append("✅ Code looks good! Consider adding more tests and documentation.")
