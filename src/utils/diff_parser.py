...
class DiffParser:
    ...
    def parse_patch(self, patch: str) -> List[Dict[str, int]]:
        ...
        for line in lines:
            if line.startswith('@@'):
                ...
                if match:
                    current_line = int(match.group(1)) - 1  # -1 because we'll increment before use
    ...
    def extract_changed_functions(self, patch: str, language: str = 'python') -> List[Dict[str, Any]]:
        """
            ...
            if line.startswith('+'):
                ...
                if match:
                    ...
                    if func_name:
                        functions.append({
                            'name': func_name,
                            'line': len([l for l in lines[:lines.index(line)] if not l.startswith('-')]),
                            'type': 'function'
                        })
    ...
    def extract_code_snippets(self, patch: str, context_lines: int = 2) -> List[Dict[str, Any]]:
        """
