...
class PRReviewBot:
    ...
    def analyze_code_with_blackbox(self, code: str, filename: str, context: str = "") -> Dict[str, Any]:
...
"""
            ...
            if response and len(response) > 50 and "Login to continue" not in response:
                ...
                logger.warning("Blackbox API returned invalid response, using local analysis only")
        ...
        try:
            ...
            for file in files:
                ...
                try:
                    ...
                    changed_lines = self.diff_parser.parse_patch(file.patch) if file.patch else []
                    ...
                    threshold_idx = severity_order.index(self.config.get('severity_threshold', 'low'))
                    
                    filtered_issues = [
                        issue for issue in file_issues
                        if severity_order.index(issue.get('severity', 'info')) >= threshold_idx
                    ]
    ...
    def _save_results(self, file_analyses: List[Dict[str, Any]], all_issues: List[Dict[str, Any]]):
